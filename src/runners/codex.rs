...
impl Runner for CodexRunner {
    ...
    fn run_turn<'a>(
        ...
        Box::pin(async move { run_codex_turn(self.runtime.clone(), ctx).await })
    }
}

async fn run_codex_turn(runtime: CodexRuntime, ctx: RunnerTurnContext) -> anyhow::Result<RunnerOutcome> {
...
mod tests {
    ...
    async fn derives_agent_message_from_codex_item() -> anyhow::Result<()> {
        ...
        let db_path = temp_dir.path().join("codex-runner.sqlite3");
    ...
    async fn auto_resolves_interaction_when_no_clients() -> anyhow::Result<()> {
        let temp_dir = tempfile::tempdir()?;
        let db_path = temp_dir.path().join("codex-runner-interaction.sqlite3");
        let db = Db::connect(&db_path).await?;
